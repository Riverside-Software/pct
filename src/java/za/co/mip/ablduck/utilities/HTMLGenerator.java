...
public class HTMLGenerator {
    private static final ResourceBundle TEMPLATES = ResourceBundle.getBundle("za.co.mip.ablduck.utilities.templates");
    ...
    public String getClassHtml (Map<String, SourceJSObject> allclasses, SourceJSObject cls) {
        ...
        return MessageFormat.format(getTemplate("CLASSBODY"), 
                                            renderSidebar(cls),
    ...
    private String renderSidebar (SourceJSObject cls) {
        if(!"".equals(cls.ext) || !cls.subclasses.isEmpty())
            return MessageFormat.format(getTemplate("SIDEBAR"), 
                                            renderClassTree(cls),
            ...
            if (member.tagname.equals(memberType)) {
                ...
                if ("method".equals(member.tagname)) {
                    ...
                    if (!member.parameters.isEmpty()) {
                        doc.append(MessageFormat.format(getTemplate("PARAMETERS"), parameters.toString()));
                    }
                ...
                memberHTML.append(MessageFormat.format(getTemplate("MEMBER"), 
                                                            member.id,          //0
                                                            firstChild,         //1
                                                            inherited,          //2
                                                            member.owner,       //3
    ...
    private String renderParams (ParameterObject renderParams) {
        return MessageFormat.format(getTemplate("PARAMETER"), renderParams.name, renderParams.datatype, renderParams.comment);
    }
    ...
    private String renderTags (MetaObject meta) {
        ...
        for (Field field : fields) {
            ...
            if (flag != null && flag)
                tags.append(MessageFormat.format(getTemplate("TAG"), key.toLowerCase(), key.toUpperCase()));
        }
    ...
    private String renderDeprecated (String tagname, DeprecatedObject deprecated) {
        return MessageFormat.format(getTemplate("DEPRECATED"), tagname, deprecated.version, deprecated.text);
    }
